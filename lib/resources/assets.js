...
module.exports = Resource.extend({
  create(assetCode, options) {
    const self = this;
    const opts = this._defaultOptions(assetCode, _.merge({
      homeDomain: null,
      mustAuthorize: true,
      isRevocable: true,
    }, options));
  ...
  _defaultOptions(assetCode, opts) {
    ...
    const authorizer = _.merge(this._defaultAccount({
      startingBalance,
    }), opts.authorizer);

    const issuerThresholds = { masterWeight: 3, lowThreshold: 1, medThreshold: 2, highThreshold: 3 };
    const issuer = _.merge(this._defaultAccount({
      startingBalance,
    ...
    if (opts.mustAuthorize) {
      issuer.signers.push({
        publicKey: options.authorizer.publicKey,
        weight: issuerThresholds.lowThreshold
      });
    }
